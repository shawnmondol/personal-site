...
export function EducationSection({ education, editMode = false, onChange }: Props) {
  ...
  return (
    <section className="section-rule" style={{padding: '40px 0'}}>
            ...
            >
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <div className={editMode ? 'flex-1 min-w-48' : ''}>
                  ...
                  />
                  {editMode ? (
                    <div className="flex flex-wrap items-baseline gap-1 text-sm text-subtle mt-1">
                      <InlineText
                        value={edu.degree}
                        editMode
                        placeholder="Degree"
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <InlineText
                    ...
                    className={`text-xs text-muted ${editMode ? 'w-20' : ''}`}
      ...
      {editMode && (
        <AddEntryButton className="mt-5" onClick={() => onChange?.([...education, {...emptyEducation}])}>
          Add Education
        </AddEntryButton>
      )}
    </section>
